...
* ~~객체 지향언어와 프로토콜 지향언어~~
  * 객체 지향언어 : 슈퍼클래스에 종속적, 하나의 슈퍼클래스만 / Value 타입을 사용 할 수 없다. 참조타입
  * 프로토콜 지향언어 : 여러 프로토콜을 따를 수 있다. / 참조타입을 사용한다.
...
* 가변매개변수
  ...
  * 5.4 이전에는 하나만 가능했지만, 5.4 이후부턴 여러개 가능해진다.
...
* 의존성 
  ...
  * 의존성 주입 : 내부가 아니라 외부에서 객체를 생성해서 넣어주는 것을 주입이라 한다.
...
* ~~앱 아키택쳐~~
  ...
  * Viper : View, Interactor(비즈니스 로직), Presenter(뷰, Interactor 데이터 전달), Entity, Router
