...
* extension
   * 프로토콜, 구조체, 클래스, 을 extension 할 수 있다. 이를 이용하여 구조체도 일관성있고 체계적인 모듈을 만들 수가 있다. 클래스는 참조타입을 하기 때문에 원본 회손의 문제가 있고 클래스만의 라이프 사이클이 있기때문에 구조체 보단 무겁다. 이때문에 Swift UI는 아에 구조체로 구성 되어있다. 
